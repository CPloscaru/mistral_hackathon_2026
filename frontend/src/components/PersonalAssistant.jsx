/**
 * PersonalAssistant - Post-onboarding page with spinner + SMART objective
 *
 * Flow:
    ...
    <div className="personal-assistant">
      ...
      {phase === 'objectif' && plan && (
        <div className="smart-display">
          ...
          {/* Show phases after typewriter finishes */}
          {!showCursor && plan.phases && (
            <div className="smart-display__phases">
              {plan.phases.map((p, idx) => (
                <div key={idx} className="smart-display__phase">
                  <div className="smart-display__phase-title">{p.titre}</div>
                  <div className="smart-display__phase-objectif">{p.objectif}</div>
                  <ul className="smart-display__phase-actions">
          ...
          {!showCursor && plan.prochaines_etapes && (
            <div className="smart-display__etapes">
              <h3>Prochaines étapes</h3>
              <ul>
                {plan.prochaines_etapes.map((etape, i) => (
                  <li key={i}>{etape}</li>
                ))}
              </ul>
            </div>
